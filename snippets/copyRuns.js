...
F.API.Archive.getRuns("saved=true&step=10&variables=$Fired^&facilitator=false&group_name=" + GROUP_TO_COPY_FROM, function(data){
	var runs = [];
	var users = [];
	...
	var start = function(data){
		...
		if(firstRun){
			...
			params = params.join("&")
			F.API.Archive.getRuns(params, start);
		}
		else{
