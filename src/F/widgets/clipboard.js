...
var ClipBoard = {
    ...
    _defaultSettings: {
        ...
        selector: ".dataTable",
        datasource: "ClipBoard.copyTable",
    ...
    copyTable: function(tableSelector, context){
        tableSelector || (tableSelector = 'table.dataTable');
        // if (!context) context = this.flashVars.context;
        ...
        if( $(tableSelector + " caption", context).size())
            tableData.push( $.trim( $(tableSelector + " caption", context).text() ) )

        $(tableSelector + " tr", context).each(function(index, row){
            ...
            if(rowspan && rowspan > 1){
                ...
                var spanSoFar = "";
                $(this).children('td:not(.skipcopy):not(.nocopy), th:not(.skipcopy):not(.nocopy)').each(function(index, cell){
                    if($(cell).attr("rowspan")){
                        var rp = parseInt($(cell).attr("rowspan")) - 1;
                        for(var j=0; j< rp; j++){
                            spanSoFar += "\t";
                            // thisnewspan.push("\t")
                        }
                        ...
                        thisnewspan.push(spanSoFar);
                        spanSoFar = "";
                    }
                });
                thisnewspan.push(spanSoFar);
                spanSoFar = "";
                newrowspanData.push(thisnewspan);

            }
            else{
                if(newrowspanData.length > 0){
                    var rowSpanForthisCell = newrowspanData.pop();
